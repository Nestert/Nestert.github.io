...
В Pages CMS откройте отдельный раздел «Порядок работ». В нём находятся перетаскиваемые списки:
...
Автоматические серии на странице «Проекты» по-прежнему упорядочиваются в разделе «Обложки серий». Список «Ручные проекты» относится только к проектам, загруженным отдельными файлами через раздел «Работы».
...
В этом же разделе поле `Порядок` является главным приоритетом расположения серий: чем меньше число, тем раньше серия показывается в «Проектах». При одинаковом порядке сначала идёт более новая серия. Если порядок не указан, используется `0`.
