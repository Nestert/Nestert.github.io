...
- `Порядок` — основной приоритет положения работы в списке;
- `Дополнительный текст страницы` — необязательный длинный текст в Markdown.

Работы сначала сортируются по полю `Порядок`: чем меньше число, тем раньше работа выводится в категории. При одинаковом значении `Порядок` сначала показывается более новый год, затем используется название. Лучше задавать значения `10`, `20`, `30`, чтобы между ними можно было вставлять новые работы.
